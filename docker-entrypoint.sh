...
then
	echo "$AWS_REGION *****"
	rabbit_set_config 'cluster_formation.aws.region' ${AWS_REGION}
else
	echo "env var AWS_REGION must be set"
	exit 1
fi

if [ "${AWS_ACCESS_KEY_ID:-}" ]; 
then
	rabbit_set_config 'cluster_formation.aws.access_key_id' ${AWS_ACCESS_KEY_ID}
else
	echo "env var AWS_ACCESS_KEY_ID must be set"
	exit 1
fi

if [ "${AWS_SECRET_KEY:-}" ]; 
then
	rabbit_set_config 'cluster_formation.aws.secret_key' ${AWS_SECRET_KEY}
else
	echo "env var AWS_SECRET_KEY must be set"
	exit 1
fi
